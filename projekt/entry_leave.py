...
class ParkingSystem:
    ...
    def process_parking_entry(self, conn, card_id, current_time):
        ...
        try:
            ...
            if existing_entry:
                ...
                duration_str = str(exit_time - entry_time).split('.')[0]  # Format HH:MM:SS

                print(f"Vehicle {card_id} exited.")
                print(f"Entry time: {entry_time.strftime('%H:%M:%S')}")
                print(f"Exit time: {exit_time.strftime('%H:%M:%S')}")
                print(f"Duration: {duration_str}")
                if bonus_hours > 0:
                    print(f"Company bonus hours used: {bonus_hours}")
                print(f"Total price: {total_price:.2f} PLN")
                ...
                print(f"Entry time: {current_time.strftime('%H:%M:%S')}")
                conn.commit()
    ...
    def calculate_parking_price(self, entry_record, bonus_hours=0):
        ...
        duration = (now - entry_time).total_seconds() / 3600  # Duration in hours
        
        # If there are bonus hours from company
        if bonus_hours > 0:
            # Calculate billable hours after subtracting bonus hours
            billable_hours = max(0, duration - bonus_hours)
            # Round up billable hours to the next hour
            ...
            # Round up to the next hour for full duration
...
def main():
    ...
    with self.connect_db() as conn:
        entry_status = self.process_parking_entry(conn, card_id, datetime.datetime.now())
